...
def configure_sc_logging(use_syslog=False):
    ...
    By default TethysCluster's logger has no formatters and a NullHandler so that
    other developers using TethysCluster as a library can configure logging as
    they see fit. This method is used in TethysCluster's application code (i.e.
    the 'tethyscluster' command) to toggle TethysCluster's application specific
    formatters/handlers
...
def get_log_for_pid(pid):
    """
    Fetches the logs from the debug log file for a given TethysCluster run by PID
    """
    found_pid = False
    pid_str = ' PID: %s ' % pid
