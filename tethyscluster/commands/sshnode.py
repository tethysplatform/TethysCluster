...
class CmdSshNode(NodeCompleter):
    ...
    Examples:
        ...
        $ tethyscluster sshnode mycluster master
        $ tethyscluster sshnode mycluster node001
        ...

    or same thing in shorthand:

        $ tethyscluster sshnode mycluster 0
        $ tethyscluster sshnode mycluster 1
        ...

    You can also execute commands without directly logging in:

        $ tethyscluster sshnode mycluster node001 'cat /etc/hosts'
    """
    names = ['sshnode', 'sn']
