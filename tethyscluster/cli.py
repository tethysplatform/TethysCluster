...
tethyscluster [global-opts] action [action-opts] [<action-args> ...]
"""
import os
import sys
import shlex
...
from tethyscluster import commands
from tethyscluster import exception
from tethyscluster import completion
...
class TethysClusterCLI(object):
    ...
    def _init_completion(self):
        """
        Restore original sys.argv from COMP_LINE in the case that tethyscluster
        is being called by Bash/ZSH for completion options. Bash/ZSH will
        simply call 'tethyscluster' with COMP_LINE environment variable set to
        the current (partial) argv for completion.

        TethysCluster's Bash/ZSH completion code needs to read the global config
        option in case an alternate config is specified at the command line
        when completing options. TethysCluster's completion code uses the config
